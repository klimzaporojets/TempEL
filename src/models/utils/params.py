...
class BlinkParser(argparse.ArgumentParser):
    ...
    def add_blink_args(self, args=None):
        ...
        parser.add_argument('--path_to_config', default=None, type=str)
        parser.add_argument(
            '--silent', action='store_true', help='Whether to print progress bars.'
        )
        parser.add_argument(
            '--json_output', action='store_true', help='Whether to produce the json output of the prediction.'
        )
        parser.add_argument(
            '--debug',
            action='store_true',
        ...
        parser.add_argument(
            ...
            # default=100,
            type=int,
            help='The number of records processed when --debug is on.',
        )
        parser.add_argument(
            '--debug_load_worlds',
            action='store_true',
            help='Whether to load all the documents during debug in order to produce a more complete json output, '
                 'if not some fields will be missing/not all asserts will be run.',
        )
        parser.add_argument(
            '--data_parallel',
            action='store_true',
        ...
        parser.add_argument(
            '--local_rank',
            type=int, default=0,
            help='The gpu id a process is executing, automatically passed when executing with '
        ...
        parser.add_argument('--nr_passages_per_doc', default=-1, type=int,
                            help='maximum number of passages that are retained for the document, -1 means all '
                                 'passages are used. In order to run it as in original config, use 1 (first passage '
                                 'of the document)')
    ...
    def add_model_args(self, args=None):
        ...
        # TODO - this pull_from_layer seems not to be used, remove?
        parser.add_argument(
        ...
        # context_key left out, think it may not be necessary
    ...
    def add_training_args(self, args=None):
        ...
        parser.add_argument(
            '--print_interval', type=int, default=1000,
